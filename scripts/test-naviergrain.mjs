...
const module = await WebAssembly.compile(readFileSync(resolve(directory, 'naviergrain-live.wasm')));
...
const live = engine.ng_live_create(48000, -1);
assert(live, 'Engine creation failed');
let energy = 0;
try {
  for (let frame = 0; frame < 96; frame++) {
    assert(engine.ng_live_render(live, 512), 'Audio render failed');
    const audio = new Float64Array(engine.memory.buffer, engine.ng_live_audio(live), 1024);
